...
function configure (app, wares, ctx, env) {
  ...
  // Remote SugarPixel alarm (lib/server/pixelalarm.js): pass-through no-ops
  // unless the feature is enabled and the caller is the configured subject.
  // The precheck decides once per request and strips If-Modified-Since so a
  // 304 can't suppress the modified payload; the apply middleware rewrites
  // res.entries just before format_entries.
  function pixel_noop (req, res, next) { next(); }
  var pixel_precheck = (ctx.pixelalarm && ctx.pixelalarm.interceptV1EntriesPrecheck) || pixel_noop;
  var pixel_apply = (ctx.pixelalarm && ctx.pixelalarm.interceptV1EntriesApply) || pixel_noop;
  var pixel_count = (ctx.pixelalarm && ctx.pixelalarm.interceptCount) || pixel_noop;

  api.use(pixel_precheck);
  /**
  ...
  }, wares.obscure_device, pixel_apply, format_entries);
   ...
   *
   */
  api.get('/entries', ifModifiedSinceCTX, query_models, wares.obscure_device, pixel_apply, format_entries);
  ...
  ```
        ...
        */
  api.get('/times/:prefix?/:regex?', prep_storage, prep_pattern_field, prep_patterns, prep_patterns, query_models, wares.obscure_device, pixel_apply, format_entries);

  api.get('/count/:storage/where', pixel_count, prep_storage, count_records, format_results);
  ...
  ```
        */
  api.get('/slice/:storage/:field/:type?/:prefix?/:regex?', prep_storage, prep_pattern_field, prep_patterns, query_models, wares.obscure_device, pixel_apply, format_entries);
