...
function configure (env, ctx) {
  ...
  // Remote SugarPixel alarm (lib/server/pixelalarm.js): answers the
  // configured subject with an empty/fake summary; a no-op otherwise.
  var pixel_intercept = (ctx.pixelalarm && ctx.pixelalarm.interceptSummary) || function pixel_noop (req, res, next) { next(); };

  api.get('/', ctx.authorization.isPermitted('api:*:read'), pixel_intercept, function (req, res) {
