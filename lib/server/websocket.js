...
function init (env, ctx, server) {
  ...
  function verifyAuthorization (message, ip, callback) {
    ...
    ctx.authorization.resolve({ api_secret: message.secret, token: message.token, ip: ip }, function resolved (err, result) {
      ...
      // Remote SugarPixel alarm (lib/server/pixelalarm.js): the DataReceivers
      // room is a broadcast of real data, so per-subject interception is not
      // possible here. While the feature is enabled, the configured display
      // subject gets no streaming read access at all - it polls HTTP, where
      // its data is properly masked.
      if (ctx.pixelalarm && result.subject && ctx.pixelalarm.isTargetSubject(result.subject.name)) {
        console.info('pixelalarm: denying websocket read access to target subject');
