...
function configure (env, ctx) {
  ...
  // Remote SugarPixel alarm (lib/server/pixelalarm.js): short-circuits the
  // pebble payload for the configured subject; a no-op for everyone else.
  var pixel_intercept = (ctx.pixelalarm && ctx.pixelalarm.interceptPebble) || function pixel_noop (req, res, next) { next(); };

  return [middle, wares.sendJSONStatus, ctx.authorization.isPermitted('api:pebble,entries:read'), pixel_intercept, pebble];
}
