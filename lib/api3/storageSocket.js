    ...
    if (message && message.accessToken) {
      return ctx.authorization.resolveAccessToken(message.accessToken, function resolveFinish (err, auth) {
        if (err) {
          ...
          // Remote SugarPixel alarm (lib/server/pixelalarm.js): storage
          // rooms broadcast real documents, so the configured display
          // subject gets no streaming access while the feature is enabled.
          if (ctx.pixelalarm && auth && auth.subject && ctx.pixelalarm.isTargetSubject(auth.subject.name)) {
            console.info('pixelalarm: denying storage socket subscription to target subject');
            if (shouldCallBack) {
              returnCallback({ success: false, message: apiConst.MSG.SOCKET_MISSING_OR_BAD_ACCESS_TOKEN });
            }
            return;
