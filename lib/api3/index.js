...
function configure (env, ctx) {
  ...
  // Remote SugarPixel alarm (lib/server/pixelalarm.js): answers GET
  // /entries* for the configured subject ahead of the generic collection
  // routes; a no-op for all other callers and collections. Mounted after
  // extension2accept so extension URLs (/entries.json) are already
  // normalized.
  if (ctx.pixelalarm) {
    app.use(ctx.pixelalarm.interceptV3);
  }
